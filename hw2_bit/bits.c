...
long replaceByte(long x, long n, long c) {
    long mask = 0xFFL << (n << 3);
    long c_shift = ((long)c) << (n << 3);
    return (x & ~mask) | c_shift;
}
/*
...
long dividePower2(long x, long n) {
    long sign = x >> 63; // sign = 0 (x >= 0), -1 (x < 0)
    long bias = (sign & ((1L << n) + (~0L)));
    return (x + bias) >> n;
}
/*
...
long isGreater(long x, long y) {
    ...
    long sx = (x >> 63) & 1;
    long sy = (y >> 63) & 1;
    long sdiff = (diff >> 63) & 1;
    long sign_diff = sx ^ sy;
    long not_equal = !!(x ^ y); // 1 if x != y, 0 if x == y
    long same_sign_case = (!sign_diff) & (!sdiff) & not_equal;
    long diff_sign_case = sign_diff & (!sx) & sy;
    return !!(same_sign_case | diff_sign_case);
}
